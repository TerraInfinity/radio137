  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (!id) return Response.json({ error: "Missing cut" }, { status: 400 });
        ...
        if (!hit) return Response.json({ error: "Cut not found" }, { status: 404 });
        const locked = isAdultTrack(hit.track) && !isChannelNsfw(hit.channel);
        if (locked) {
          const user = await resolveRadioUser();
          if (!user) return Response.json({ error: "Sign in to download this cut" }, { status: 401 });
        }
