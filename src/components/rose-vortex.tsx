...
function wrapCopy(ctx: CanvasRenderingContext2D, text: string, maxW: number, maxLines = 3): string[] {
  ...
  while (i < words.length && out.length < maxLines) {
    let line = words[i] ?? "";
    i += 1;
    while (i < words.length && out.length < maxLines - 1) {
    ...
    if (out.length === maxLines - 1) {
      ...
      if (i < words.length && line.length > 1) line = `${line.replace(/\s+\S*$/, "")}…`;
    }
    out.push(line);
  }
  return out;
...
function paintCopyBlock(
  ctx: CanvasRenderingContext2D,
  text: string,
  lane: { x: number; y: number; align: CanvasTextAlign; wide: number },
  w: number,
  h: number,
  size: number,
  alpha: number,
  ...
  const maxW = w * lane.wide;
  ctx.save();
  ctx.font = font.replace("SIZE", String(size));
  const rows = wrapCopy(ctx, text, maxW, plate ? 3 : 2);
  if (!rows.length) {
    ctx.restore();
    return;
  }
  const gap = size * 1.18;
  const blockH = gap * rows.length;
  const blockW = Math.min(maxW, Math.max(...rows.map((row) => ctx.measureText(row).width)));
  const x = w * lane.x;
  const y = h * lane.y;
  if (plate) {
    const padX = 16;
    const padY = 10;
    let left = x - padX;
    if (lane.align === "center") left = x - blockW / 2 - padX;
    if (lane.align === "right") left = x - blockW - padX;
    ctx.globalAlpha = Math.min(0.86, alpha);
    ctx.fillStyle = "rgba(7, 3, 10, 0.78)";
    ctx.beginPath();
    ctx.roundRect(left, y - blockH / 2 - padY, blockW + padX * 2, blockH + padY * 2, 12);
    ctx.fill();
  }
  ctx.globalAlpha = alpha;
  ...
  ctx.strokeStyle = "rgba(8, 2, 8, 0.92)";
  ctx.fillStyle = fill;
  ctx.shadowColor = "rgba(0, 0, 0, 0.8)";
  ctx.shadowBlur = 10;
  rows.forEach((row, index) => {
    const ly = y - blockH / 2 + gap * (index + 0.5);
...
function drawRedGlare(ctx: CanvasRenderingContext2D, w: number, h: number, pulse: RosePulse) {
  const open = 0.42 + pulse.kick * 0.4 + pulse.downbeat * 0.18;
  const x = w * 0.78;
  const y = h * 0.2;
  const s = Math.min(w, h) * 0.034;
  const flick = (pulse.snare > 0.25 ? 0.42 : pulse.beatPhase * 0.2) * s;
  const tilt = -0.42 + Math.sin(pulse.phrasePhase * Math.PI * 2) * 0.06;
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(tilt);
  ctx.globalAlpha = Math.min(0.92, open);
  ctx.fillStyle = "#140308";
  ctx.beginPath();
  ctx.ellipse(0, 0, s * 2.4, s * 0.62, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "rgba(251, 247, 243, 0.45)";
  ctx.lineWidth = Math.max(1, s * 0.08);
  ctx.stroke();
  ctx.fillStyle = "#c4233a";
  ctx.shadowColor = "#ff4d62";
  ctx.shadowBlur = 8 + pulse.kick * 14;
  ctx.beginPath();
  ctx.ellipse(flick, s * 0.02, s * 0.48, s * 0.22, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.shadowBlur = 0;
  ctx.fillStyle = "#140308";
  ctx.fillRect(flick - s * 0.045, -s * 0.2, Math.max(1.2, s * 0.09), s * 0.44);
  ctx.fillStyle = "#fff4ee";
  ctx.globalAlpha *= 0.9;
  ctx.beginPath();
  ctx.arc(flick + s * 0.16, -s * 0.05, s * 0.07, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = Math.min(0.75, open * 0.7);
  ctx.translate(-s * 2.7, -s * 0.85);
  ctx.rotate(0.18);
  ctx.fillStyle = "#140308";
  ctx.beginPath();
  ctx.ellipse(0, 0, s * 1.55, s * 0.38, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#9d1c32";
  ctx.beginPath();
  ctx.ellipse(flick * 0.4, 0, s * 0.22, s * 0.1, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
}
...
const SPRITES = {
  ...
  timewar: "/experiences/rose/timewar.jpg",
  ...
  prettyEyes: "/experiences/rose/eyes.jpg",
...
/** Chapel dark, Time War sails, the eyes that pull the sun out of the wire. */
function drawShiny(
  ctx: CanvasRenderingContext2D,
  w: number,
  h: number,
  pulse: RosePulse,
  energy: number,
  war: HTMLImageElement,
  tunnel: HTMLImageElement,
  eyes: HTMLImageElement,
  rose: HTMLImageElement,
) {
  coverBlit(ctx, war, w, h, 0.35 + pulse.phrasePhase * 0.2, 1);
  coverBlit(ctx, tunnel, w, h, 0.6, 0.22 + energy * 0.15);
  const lamp = ctx.createRadialGradient(w * 0.5, h * 0.42, 10, w * 0.5, h * 0.42, Math.min(w, h) * 0.42);
  lamp.addColorStop(0, `rgba(255, 214, 140, ${0.16 + pulse.downbeat * 0.12})`);
  lamp.addColorStop(1, "rgba(40, 8, 6, 0)");
  ctx.fillStyle = lamp;
  ctx.fillRect(0, 0, w, h);
  ctx.save();
  ctx.globalAlpha = 0.35 + energy * 0.25;
  ctx.strokeStyle = "rgba(196, 92, 48, 0.8)";
  ctx.lineWidth = 1.4;
  for (let i = 0; i < 5; i++) {
    const y = h * (0.2 + i * 0.12) + Math.sin(pulse.time * 0.4 + i) * 10;
    const lean = (i - 2) * w * 0.08;
    ctx.beginPath();
    ctx.moveTo(w * 0.08 + lean, y + 40);
    ctx.quadraticCurveTo(w * 0.5, y - 30, w * 0.92 - lean, y + 20);
    ctx.stroke();
  }
  ctx.restore();
  if (ready(eyes)) {
    const dw = Math.min(w * 0.46, h * 0.7);
    const dh = dw * (eyes.naturalHeight / Math.max(1, eyes.naturalWidth));
    featherPortrait(ctx, eyes, w * 0.5, h * 0.46, dw, dh, 0.82);
  }
  if (ready(rose)) {
    const dw = Math.min(w * 0.16, 140);
    const dh = dw * (rose.naturalHeight / Math.max(1, rose.naturalWidth));
    featherPortrait(ctx, rose, w * 0.82, h * 0.72, dw, dh, 0.7);
  }
  drawActLine(ctx, stageLines("shiny"), pulse, w, h, Math.max(16, Math.min(28, w * 0.028)));
}

/** A loud commercial. The rotor is already seasoned. There is too much shrimp. */
function drawTimeshare(
  ...
  coverBlit(ctx, ship, w, h, 0.2, 0.55);
  ctx.fillStyle = "rgba(6, 10, 16, 0.4)";
  ctx.fillRect(0, 0, w, h);
  const cx = w * 0.5;
  const cy = h * 0.42;
  if (ready(copter)) {
    const dw = Math.min(w * 0.7, h * 0.85);
    const dh = dw * (copter.naturalHeight / Math.max(1, copter.naturalWidth));
    const hop = Math.sin(pulse.time * 1.4) * 8 + pulse.kick * 10;
    featherPortrait(ctx, copter, cx, cy + hop, dw, dh, 0.9);
  }
  ctx.save();
  ctx.translate(cx, cy - h * 0.12);
  ctx.rotate(pulse.time * (2.2 + energy));
  ctx.strokeStyle = `rgba(255, 186, 120, ${0.45 + pulse.kick * 0.4})`;
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-w * 0.28, 0);
  ctx.lineTo(w * 0.28, 0);
  ...
  if (ready(shrimp)) {
    for (let i = 0; i < 6; i++) {
      const a = pulse.time * 0.7 + i * 1.05;
      const dw = 70 + (i % 3) * 18;
      const dh = dw * (shrimp.naturalHeight / Math.max(1, shrimp.naturalWidth));
      featherPortrait(ctx, shrimp, cx + Math.cos(a) * w * 0.32, cy + Math.sin(a) * h * 0.22, dw, dh, 0.55);
    }
  }
  const stripe = (pulse.beatPhase + pulse.downbeat) % 1;
  ctx.fillStyle = `rgba(255, 80, 60, ${0.12 + stripe * 0.2})`;
  ctx.fillRect(0, h * 0.78, w, 10);
  drawActLine(ctx, stageLines("timeshare"), pulse, w, h, Math.max(16, Math.min(28, w * 0.028)));
}

/** The car never stops. Tiny hands and soft shells rise with it. */
function drawElevate(
  ...
  const shaftL = w * 0.28;
  const shaftR = w * 0.72;
  ctx.fillStyle = "rgba(40, 16, 28, 0.9)";
  ctx.fillRect(shaftL, 0, shaftR - shaftL, h);
  const rise = (pulse.time * (18 + energy * 30)) % (h * 0.22);
  ctx.strokeStyle = "rgba(232, 168, 190, 0.35)";
  ctx.lineWidth = 2;
  for (let i = -1; i < 8; i++) {
    const y = (i * h * 0.22 - rise + h) % h;
    ctx.beginPath();
    ctx.moveTo(shaftL, y);
    ctx.lineTo(shaftR, y);
    ctx.stroke();
  ...
  const cabin = h * 0.34;
  ctx.strokeStyle = `rgba(255, 214, 220, ${0.5 + pulse.kick * 0.3})`;
  ctx.strokeRect(shaftL + 16, cabin, shaftR - shaftL - 32, h * 0.38);
  const riders = [bloom, rose, fairy];
  riders.forEach((img, i) => {
    if (!ready(img)) return;
    const dw = Math.min(w * 0.22, 180);
    const dh = dw * (img.naturalHeight / Math.max(1, img.naturalWidth));
    const y = cabin + h * 0.16 + Math.sin(pulse.time * 0.8 + i) * 6;
    featherPortrait(ctx, img, shaftL + (i + 1) * ((shaftR - shaftL) / 4), y, dw, dh, 0.8);
  });
  drawActLine(ctx, stageLines("elevate"), pulse, w, h, Math.max(16, Math.min(28, w * 0.028)));
}

/** Two shells, one road, a heart kept in the inner chamber. */
function drawShell(
  ctx: CanvasRenderingContext2D,
  w: number,
  h: number,
  ...
  if (ready(cat)) {
    ...
    featherPortrait(ctx, cat, w * 0.5 - gap, h * 0.48, dw, dh, 0.75);
  }
  if (ready(rose)) {
    const dw = Math.min(w * 0.28, 220) * (0.7 + lock * 0.4);
    const dh = dw * (rose.naturalHeight / Math.max(1, rose.naturalWidth));
    featherPortrait(ctx, rose, w * 0.5 + gap * 0.4, h * 0.5, dw, dh, 0.55 + lock * 0.4);
  }
  if (ready(petal) && lock > 0.7) {
    const dw = 90;
    const dh = dw * (petal.naturalHeight / Math.max(1, petal.naturalWidth));
    featherPortrait(ctx, petal, w * 0.5, h * 0.5, dw, dh, (lock - 0.7) * 2);
  }
  drawActLine(ctx, stageLines("shell"), pulse, w, h, Math.max(16, Math.min(28, w * 0.028)));
}

/** The figure, as photographed. No circular mask. */
function placeInStorm(
  ctx: CanvasRenderingContext2D,
  img: HTMLImageElement,
  cx: number,
  cy: number,
  ...
  featherPortrait(ctx, img, cx, cy, dw, dh, alpha);
}
...
function drawArrival(
  ...
  if (girl) {
    ...
    placeInStorm(ctx, girl, w * spot.x, h * spot.y + bob, dw, dh, 0.94 * alpha);
  }
  ...
  if (ready(pal)) {
    ...
    placeInStorm(ctx, pal, w * side, h * (slot % 2 === 0 ? 0.74 : 0.3) - bob * 0.4, pw, ph, 0.72);
  }
  if (slot === 0 && ready(sword)) {
    const sh = h * 0.26;
    const sw = sh * (sword.naturalWidth / Math.max(1, sword.naturalHeight));
    placeInStorm(ctx, sword, w * 0.14, h * 0.7, sw, sh, 0.4);
  }
...
export type RoseRock = { charge: number; hits: number; burst: number };

function drawRockBonus(
  ctx: CanvasRenderingContext2D,
  kind: string,
  w: number,
  h: number,
  ...
  const charge = rock.charge;
  if (charge < 0.04 && rock.burst < 0.04) return;
  const spice = rock.hits % 3;
  const cx = w * 0.5;
  const cy = h * 0.46;
  const kick = 0.35 + pulse.kick * 0.65 + rock.burst;
  ctx.save();
  ctx.globalCompositeOperation = spice === 2 ? "lighter" : "screen";
  ctx.globalAlpha = Math.min(0.9, 0.15 + charge * 0.55 + rock.burst * 0.35);

  if (kind === "prom" || kind === "copter" || kind === "timeshare") {
    for (let i = 0; i < 18; i++) {
      const a = (i / 18) * Math.PI * 2 + pulse.time * (spice ? 1.4 : -0.8);
      const r = (40 + charge * 180) * (0.4 + (i % 5) * 0.12) * kick;
      ctx.fillStyle = i % 2 ? "#ff4d6a" : "#f6e2b3";
      ctx.fillRect(cx + Math.cos(a) * r, cy + Math.sin(a) * r * 0.62, 4 + spice * 3, 10 + charge * 16);
    }
  } else if (kind === "twist" || kind === "obay" || kind === "current") {
    const slices = 6 + spice * 4;
    for (let i = 0; i < slices; i++) {
      const y = ((i + pulse.beatPhase) / slices) * h;
      const shift = Math.sin(pulse.time * 18 + i) * (12 + charge * 48);
      ctx.fillStyle = i % 2 ? "rgba(255, 40, 70, 0.35)" : "rgba(255, 244, 230, 0.18)";
      ctx.fillRect(shift, y, w, h / slices - 2);
    }
  } else if (kind === "remember" || kind === "recall" || kind === "sweetie" || kind === "vow" || kind === "manual" || kind === "shell" || kind === "elevate") {
    const rings = 3 + spice;
    for (let i = 0; i < rings; i++) {
      const r = (30 + i * 46) * (0.6 + charge) + rock.burst * 40;
      ctx.strokeStyle = i % 2 ? "#f2c14e" : "#fff6ea";
      ctx.lineWidth = 2 + charge * 4;
      ctx.beginPath();
      ctx.ellipse(cx, cy, r, r * 0.72, pulse.time * 0.4, 0, Math.PI * 2);
      ctx.stroke();
    }
  } else if (kind === "firewall" || kind === "wolf" || kind === "badend" || kind === "shiny") {
    ctx.strokeStyle = "#ff2a45";
    ctx.lineWidth = 3 + charge * 6;
    const rays = 5 + spice * 3;
    for (let i = 0; i < rays; i++) {
      const a = (i / rays) * Math.PI + pulse.time * (spice === 1 ? -1.2 : 0.9);
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.lineTo(cx + Math.cos(a) * w * (0.2 + charge * 0.45), cy + Math.sin(a) * h * 0.35);
      ctx.stroke();
    }
  } else if (kind === "allocate" || kind === "choir" || kind === "halo") {
    for (let i = 0; i < 7; i++) {
      const x = w * (0.15 + (i / 6) * 0.7) + Math.sin(pulse.time * 6 + i) * 10 * charge;
      ctx.strokeStyle = i % 2 ? "#ffd36a" : "#ff4d6a";
      ctx.lineWidth = 1 + charge * 2;
      ctx.beginPath();
      ctx.moveTo(x, 0);
      ctx.lineTo(x + Math.sin(pulse.time * 8 + i) * 24, h);
      ctx.stroke();
    }
  } else if (kind === "stillhot") {
    for (let i = 0; i < 14; i++) {
      const x = (i / 14) * w;
      const rise = ((pulse.time * (40 + charge * 80) + i * 30) % (h + 40)) - 20;
      ctx.fillStyle = "#ff5a3c";
      ctx.fillRect(x, h - rise, 3 + spice * 2, 18 + charge * 28);
    }
  } else {
    const rings = 2 + spice;
    for (let i = 0; i < rings; i++) {
      const r = (50 + i * 70 + rock.burst * 90) * (0.5 + charge);
      ctx.strokeStyle = i === spice ? "#ff3355" : "#ffe9c4";
  ...
  if (rock.burst > 0.45) {
    const word = spice === 0 ? "ROCK" : spice === 1 ? "BLAST" : "PUMP";
    ctx.globalAlpha = rock.burst * 0.85;
    ctx.fillStyle = "#ff2040";
    ctx.font = `700 ${Math.min(w * 0.16, 120)}px Impact, "Arial Narrow", sans-serif`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(word, cx, h * 0.62);
  }
  ctx.restore();
}
...
export function RoseVortex({
  ...
  useEffect(() => {
    ...
    const prettyEyesImg = loadSprite(SPRITES.prettyEyes);
    ...
    const draw = (now: number) => {
      ...
      rockNow.burst = Math.max(0, rockNow.burst - dt * 1.35);
      const amp = 1 + rockNow.charge * 2.15;
      let energy = pulse.energy * look.intensity * amp;
      let fly = (live ? pulse.flying : 0.05) * look.fly * (kind === "void" ? 0.35 : kind === "still-rite" ? 0.4 : kind === "prom" || kind === "remember" ? 0.45 : kind === "firewall" ? 0.7 : kind === "twist" ? 1.05 : 1) * (1 + rockNow.charge * 1.4);
      let rush = (chasing
        ? (live ? 1.35 + fly * 1.7 + pulse.kick * 0.85 : 0.72)
        : live ? 0.28 + fly * 1.15 : 0.08) * (1 + rockNow.charge * 0.85);
      ...
      if (chasing) {
        ...
        cam.trauma = Math.min(1, cam.trauma + (live ? pulse.kick * (storm ? 0.12 : 0.48) : 0) + rockNow.burst * 0.55);
      ...
      } else if (kind === "remember" || kind === "vow") {
      ...
      if (kind === "timeshare") {
        drawTimeshare(ctx, w, h, pulse, energy, allocateCopterImg, rememberShrimpImg, copterShipImg);
      }
      if (kind === "elevate") {
      ...
      if (kind === "firewall" || kind === "allocate" || kind === "current" || kind === "halo" || kind === "choir" || kind === "badend") {
      ...
      for (const glyph of glyphsRef.current) {
        ...
        ctx.strokeStyle = `rgba(243, 230, 210, ${0.14 + (1 - glyph.z) * 0.32})`;
        ctx.lineWidth = 1;
        ctx.beginPath();
      ...
      if (kind === "arrival") {
        const tardis = project(tardisX, tardisY, tardisZ, cx, cy, fov);
        const hgt = Math.min(w, h) * (0.2 / Math.max(0.95, tardisZ));
        const lamp = Math.min(1, visKick * 0.85 + pulse.downbeat + 0.55);
        drawTardisCraft(ctx, tardisChase, tardisRear, tardis.x, tardis.y, hgt, lamp, bank);
      }

      if (stage) {
        const papers = paperRef.current;
        const palette = kind === "twist" ? TWIST_PAPER : kind === "remember" || kind === "vow" ? REMEMBER_PAPER : kind === "recall" ? RECALL_PAPER : kind === "obay" ? OBAY_PAPER : kind === "copter" ? COPTER_PAPER : kind === "firewall" ? FIREWALL_PAPER : kind === "allocate" ? ALLOCATE_PAPER : kind === "wolf" ? WOLF_PAPER : kind === "current" ? CURRENT_PAPER : kind === "sweetie" ? SWEETIE_PAPER : kind === "halo" ? HALO_PAPER : kind === "choir" ? CHOIR_PAPER : kind === "badend" ? BADEND_PAPER : PROM_PAPER;
      ...
      if (kind !== "void" && kind !== "arrival" && kind !== "remember" && kind !== "vow" && kind !== "recall" && kind !== "obay" && kind !== "copter" && kind !== "stillhot" && kind !== "firewall" && kind !== "allocate" && kind !== "wolf" && kind !== "current" && kind !== "sweetie" && kind !== "halo" && kind !== "choir" && kind !== "badend" && kind !== "manual" && kind !== "shiny" && kind !== "timeshare" && kind !== "elevate" && kind !== "shell") {
