...
export const pullStationLyrics = createServerFn({ method: "POST" })
  .middleware([adminMiddleware])
  .validator((input: unknown) =>
    z
      .object({
  ...
  .handler(async ({ context, data }) => {
    const { sunoPlaylistId, pairLyrics, withLyricsSource } = await import("@/lib/stage-lines");
    const id = sunoPlaylistId(data.url);
    if (!id) throw new Error("Use a Suno playlist link, like suno.com/playlist/…");
    const response = await fetch(`https://studio-api.prod.suno.com/api/playlist/${id}/`, {
      headers: { accept: "application/json", "user-agent": "TerraInfinityRadio/1.0" },
    });
    if (!response.ok) throw new Error("Suno did not return that playlist.");
    const body = (await response.json()) as {
      playlist_clips?: { clip?: { title?: string; metadata?: { prompt?: string } } }[];
    };
    const clips = (body.playlist_clips ?? [])
      .map((item) => ({
        title: item.clip?.title ?? "",
        lyrics: item.clip?.metadata?.prompt ?? "",
      }))
      .filter((clip) => clip.title && clip.lyrics);
    const { listEdits, listStationEdits, patchTrack, upsertStation } = await import("@/lib/catalog-edits.server");
    const { applyCatalogEdits } = await import("@/lib/catalog-edits");
    const { getPlayableTracks, getSeedCatalog } = await import("@/lib/catalog");
    const { lookForTrack, mergeSceneTags, sceneFromTags } = await import("@/lib/phenomena");
    const { lookFromStation } = await import("@/lib/rose-look");
    const { experienceFromChannel } = await import("@/lib/experiences");
    const catalog = applyCatalogEdits(getSeedCatalog(), await listEdits(), await listStationEdits());
    const channel = catalog.channels.find((item) => item.slug === data.channelSlug);
    if (!channel) throw new Error("Station not found.");
    const tracks = getPlayableTracks(channel);
    const pairs = pairLyrics(tracks, clips, Boolean(data.force));
    ...
    for (const pair of pairs) {
      const index = tracks.findIndex((item) => item.id === pair.track.id);
      const scene = sceneFromTags(pair.track.tags);
      const current = lookForTrack(base, pair.track, Math.max(0, index));
      await patchTrack(context.user, {
        channelSlug: channel.slug,
        trackId: pair.track.id,
        tags: mergeSceneTags(pair.track.tags, { ...current, ...scene, captions: pair.lines }),
      });
    }
    await upsertStation(context.user, { slug: channel.slug, tags: withLyricsSource(channel.tags, data.url) });
    const missed = tracks
      .filter((track) => !pairs.some((pair) => pair.track.id === track.id))
      .map((track) => track.title);
    return { matched: pairs.map((pair) => ({ title: pair.track.title, lines: pair.lines.length })), missed, ...(await snapshot()) };
  });
