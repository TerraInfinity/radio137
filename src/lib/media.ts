import { phenomenonFromTitle, phenomenonMeta, sceneFromTags } from "@/lib/phenomena";
...
const TITLE_PORTRAITS: Array<[RegExp, string]> = [
  [/always find you/, "/experiences/rose/remember-doors.jpg"],
  [/chaos manual|perpetual chaos/, "/experiences/rose/allocate-cleo.jpg"],
  [/helicopter|shrimp ad/, "/experiences/rose/allocate-copter.jpg"],
  [/routine elevation|glom glom|elevator/, "/experiences/rose/rose-bloom.jpg"],
  [/soft shell/, "/experiences/rose/sweetie-cat.jpg"],
];

function portraitFromTitle(title?: string | null): string {
  const t = (title ?? "").toLowerCase();
  for (const [pattern, src] of TITLE_PORTRAITS) {
    if (pattern.test(t)) return src;
  }
  return "";
}

/** A song's own still, or the scene artwork made for that cut. */
export function songPortrait(
  track?: { title?: string | null; coverUrl?: string | null; tags?: string[] | null } | null,
  channel?: { cover?: string | null } | null,
): string {
  const song = (track?.coverUrl ?? "").trim();
  const station = (channel?.cover ?? "").trim();
  if (song && song !== station && !isLoopingVisual(song)) return song;
  const named = portraitFromTitle(track?.title);
  if (named) return named;
  const id = sceneFromTags(track?.tags)?.phenomenon || phenomenonFromTitle(track?.title);
  if (id) return phenomenonMeta(id).thumb;
  return song || station;
}
