...
import { lyricsForTitle, sameLines } from "./stage-lines.ts";
...
export const PHENOMENA = [
  ...
  { id: "vow", label: "Always find you", hint: "Two seekers. Blue doors. They meet.", thumb: "/experiences/rose/eyes.jpg" },
  ...
  { id: "manual", label: "Chaos manual", hint: "A nested book. Each page a softer shell. The storm learns to nest.", thumb: "/experiences/rose/allocate-cleo.jpg" },
  { id: "shiny", label: "Pretty shiny eyes", hint: "A chapel, a Time War, sails of rust and gold. The eyes remember.", thumb: "/experiences/rose/timewar.jpg" },
  { id: "timeshare", label: "Shrimp timeshare", hint: "The ad. A pre-seasoned rotor. Too much shrimp.", thumb: "/experiences/rose/allocate-copter.jpg" },
  { id: "elevate", label: "Routine elevation", hint: "An elevator that never stops. It only rises.", thumb: "/experiences/rose/rose-bloom.jpg" },
  { id: "shell", label: "Soft shell home", hint: "Two shells. One road. A backup kept in the chest.", thumb: "/experiences/rose/sweetie-cat.jpg" },
...
export function isChaosManual(title?: string | null): boolean {
  const t = (title ?? "").toLowerCase();
  return t.includes("chaos manual") || (t.includes("beauty") && t.includes("scary"));
}

export function isTimeShare(title?: string | null): boolean {
  const t = (title ?? "").toLowerCase();
  if (t.includes("bootie") || t.includes("shake that")) return false;
  return t.includes("helicopter") || t.includes("timeshare") || t.includes("time share") || t.includes("shrimp ad");
}

export function isElevation(title?: string | null): boolean {
  const t = (title ?? "").toLowerCase();
  if (t.includes("swooning") || t.includes("bootie")) return false;
  return t.includes("routine elevation") || t.includes("elevator") || t.includes("glom glom");
}

export function isSoftShell(title?: string | null): boolean {
  const t = (title ?? "").toLowerCase();
  if (t.includes("sweetie")) return false;
  return t.includes("soft shell") || t.includes("backed you up");
}

export function isShinyEyes(title?: string | null): boolean {
  const t = (title ?? "").toLowerCase();
  if (t.includes("avatar") || t.includes("river") || t.includes("wolf") || t.includes("sweetie") || t.includes("always find")) return false;
  return t.includes("shiny");
}
...
export function isShakeBootie(title?: string | null): boolean {
  const t = (title ?? "").toLowerCase();
  if (t.includes("chaos manual")) return false;
...
export function isAlwaysFind(title?: string | null): boolean {
  const t = (title ?? "").toLowerCase();
  return t.includes("always find");
}

export function isPrettyAvatar(title?: string | null): boolean {
  const t = (title ?? "").toLowerCase();
  if (t.includes("shiny")) return false;
...
export function isStageOwned(id: string | undefined | null): boolean {
  return id === "prom" || id === "twist" || id === "remember" || id === "vow" || id === "firewall" || id === "allocate" || id === "wolf" || id === "current" || id === "sweetie" || id === "halo" || id === "choir" || id === "badend" || id === "recall" || id === "obay" || id === "copter" || id === "manual" || id === "shiny" || id === "timeshare" || id === "elevate" || id === "shell";
}

export function phenomenonFromTitle(title?: string | null): PhenomenonId | null {
  if (isChaosManual(title)) return "manual";
  if (isTimeShare(title)) return "timeshare";
  if (isElevation(title)) return "elevate";
  if (isSoftShell(title)) return "shell";
  if (isShinyEyes(title)) return "shiny";
  ...
  if (isAlwaysFind(title)) return "vow";
...
export function lookForTrack(base: RoseLook, track: Track | null | undefined, index: number): RoseLook {
  ...
  const hand = Boolean(track?.tags?.includes("lines:hand"));
  const custom = hand || (sceneLines.length > 0 && !sameLines(sceneLines, stock));
  const sung = custom ? sceneLines : lyricsForTitle(track?.title);
  return normalizeLook({
    ...next,
    ...scene,
    phenomenon,
    captions: sung?.length ? sung : next.captions,
