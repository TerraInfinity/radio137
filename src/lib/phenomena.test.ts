...
describe("phenomena", () => {
  ...
  it("gives the still-only songs their own scenes", () => {
    assert.equal(phenomenonFromTitle("Lady Glaum's Chaos Manual"), "manual");
    assert.equal(phenomenonFromTitle("Pretty Face, Pretty Shiny Eyes"), "shiny");
    assert.equal(phenomenonFromTitle("Helicopter Time Share Shrimp Ad"), "timeshare");
    assert.equal(phenomenonFromTitle("Glaum Shrimp Prom Routine Elevation"), "elevate");
    assert.equal(phenomenonFromTitle("Soft Shell Home"), "shell");
    assert.equal(phenomenonFromTitle("Pretty Face, Pretty Avatar Eyes (From The River to the C)"), "firewall");
    assert.equal(phenomenonFromTitle("Glaum Shrimp Prom — Swooning for Glaum's Attention"), "prom");
    assert.equal(phenomenonFromTitle("Shake That Bootie"), "copter");
  });

  it("gives Always Find You its own scene", () => {
    assert.equal(phenomenonFromTitle("Pretty Face, Pretty Eyes I will always find you"), "vow");
    assert.equal(phenomenonFromTitle("Pretty Face, Pretty Avatar Eyes (From The River to the C)"), "firewall");
    assert.equal(phenomenonFromTitle("Rose Remembers!"), "remember");
  });
  ...
  it("steps through every graphic and wraps", () => {
    ...
    assert.equal(stepPhenomenon("stillhot"), "manual");
    assert.equal(stepPhenomenon("shell"), "arrival");
