...
import { c as init_dist_es, l as NodeHttpHandler } from "./client-s3+[...].mjs";
...
Set AWS_CONTAINER_CREDENTIALS_FULL_URI or AWS_CONTAINER_CREDENTIALS_RELATIVE_URI.`, { logger: options.logger });
	...
	const provider = retryWrapper(async () => {
		...
		if (tokenFile) request.headers.Authorization = validateToken((await fsPromises.readFile(tokenFile)).toString());
...
var validateToken = (token) => {
	if (token.includes("\r\n")) throw new import_config.CredentialsProviderError("Authorization token contains invalid \\r\\n sequence.");
	return token;
};
//#endregion
