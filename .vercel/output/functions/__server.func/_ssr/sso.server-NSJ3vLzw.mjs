...
//#region node_modules/.nitro/vite/services/ssr/assets/sso.server-NSJ3vLzw.js
...
async function resolveRadioUser(bearerToken) {
	...
	const { getSessionUser } = await import("./verify.server-gW0aWp5S.mjs");
...
async function requireRadioUser(bearerToken) {
	...
	if (!user) {
		const { UnauthorizedError } = await import("./verify.server-gW0aWp5S.mjs");
...
function envLamps() {
	...
	return [
		...
		},
		{
			...
			required: true,
			hint: "Supabase pooler (*.pooler.supabase.com) on Vercel. Direct db.<ref>.supabase.co is often IPv6-only."
		},
		{
			key: "R2_ACCOUNT_ID",
			label: "R2 account",
		...
		},
		{
			...
			group: "R2",
			set: present("R2_PUBLIC_BASE_URL") || present("MEDIA_PUBLIC_BASE"),
			required: false,
			hint: "Or MEDIA_PUBLIC_BASE / VITE_MEDIA_PUBLIC_BASE. Flip the host later to move buckets; paths stay radio/…"
		},
		{
			key: "VITE_MEDIA_PUBLIC_BASE",
			label: "Media public base",
			group: "R2",
			set: present("VITE_MEDIA_PUBLIC_BASE"),
			required: false,
			hint: "Browser rewrite for stored r2.terrainfinity.ca / *.r2.dev URLs. Same value as MEDIA_PUBLIC_BASE when you migrate."
		},
		{
			key: "ADMIN_EMAILS",
