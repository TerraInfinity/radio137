...
//#region node_modules/.nitro/vite/services/ssr/assets/similar-cuts-BE0d4_OA.js
var similar_cuts_BE0d4_OA_exports = /* @__PURE__ */ __exportAll({
	n: () => similarClusters,
	r: () => similar_cuts_exports,
	t: () => playlistDuplicateHints
});
var similar_cuts_exports = /* @__PURE__ */ __exportAll$1({
	clusterSkipKeys: () => clusterSkipKeys,
	playlistDuplicateHints: () => playlistDuplicateHints,
	similarClusters: () => similarClusters,
	skipPairKey: () => skipPairKey
});
...
function skipPairKey(a, b) {
	return a < b ? `${a}|${b}` : `${b}|${a}`;
}
function clusterSkipKeys(ids) {
	const unique = [...new Set(ids.filter(Boolean))].sort();
	const keys = [];
	for (let i = 0; i < unique.length; i += 1) for (let j = i + 1; j < unique.length; j += 1) keys.push(`${unique[i]}|${unique[j]}`);
	return keys;
}
function fold(value) {
...
function playlistStripped(value) {
	return fold(value).replace(/^\d{1,2}\s+/, "");
}
function stripNoise(value, extra = []) {
	const drop = /* @__PURE__ */ new Set([...GENERIC, ...extra.flatMap((item) => fold(item).split(" ").filter(Boolean))]);
	return tokens(playlistStripped(value)).filter((token) => !drop.has(token)).join(" ");
}
function compact(value) {
	return fold(value).replace(/\s+/g, "");
}
function stripSequel(value) {
	return playlistStripped(value).replace(/\bv\s*\d+$/g, "").replace(/\b(pt|part|vol|volume)\s*\d+$/g, "").replace(/\s+\d+$/g, "").trim();
}
...
var DUP_EXTRA = /* @__PURE__ */ new Set([
	"copy",
	"duplicate",
	"dup",
	"alt",
...
function coreOf(copy) {
	...
	const drop = /* @__PURE__ */ new Set(["glados", ...extra.flatMap((item) => fold(item).split(" ").filter(Boolean))]);
	const raw = compact(tokens(playlistStripped(copy.track.title)).filter((token) => !drop.has(token)).join(" "));
	return {
		title: title || fold(copy.track.title),
		stem: stem || fold(copy.stem),
...
function durationClose(a, b) {
	...
	return delta <= Math.max(2, floor * .03);
}
function isSequelPair(a, b) {
...
function scorePair(left, right) {
	...
	if (identityNumbers(left.track.title).sort().join() !== identityNumbers(right.track.title).sort().join()) return null;
	...
	const packed = Math.max(ratio(coreLeft.compact, coreRight.compact), ratio(coreLeft.raw, coreRight.raw), dice(coreLeft.compact, coreRight.compact), dice(coreLeft.raw, coreRight.raw));
	...
	const extra = leftTokens.length <= rightTokens.length ? extraMeaningful(leftTokens, rightTokens) : extraMeaningful(rightTokens, leftTokens);
	const credit = isCreditSuffix(coreLeft.title, coreRight.title) || isCreditSuffix(coreRight.title, coreLeft.title);
	if (extra.length > 0 && !credit && packed < .9) return null;
	let score = Math.max(titleRatio, stemRatio, cross, packed, overlap);
	if (lastTokenClash(leftTokens, rightTokens)) score -= .18;
	if (credit) score = Math.max(score, .9);
	const artist = artistKey(left);
	const sameArtist = Boolean(artist) && artist === artistKey(right);
	if (sameArtist) score += .05;
	const sameSource = Boolean(left.track.originalUrl) && left.track.originalUrl === right.track.originalUrl;
	if (sameSource) score += .12;
	...
	const compactEnough = Math.min(Math.max(coreLeft.compact.length, coreLeft.raw.length), Math.max(coreRight.compact.length, coreRight.raw.length)) >= 8;
	if (!(packed >= .9 && compactEnough || credit || overlap >= .72 && score >= .7 && extra.length === 0 || score >= (short ? .9 : .86) && extra.length === 0)) return null;
	if (sameStation && extra.length > 0 && !credit && packed < .96) return null;
	if (disjoint && packed < .9) return null;
	const why = [];
	if (credit) why.push("close titles");
	else if (packed >= .9 || titleRatio >= .82) why.push("close titles");
	else if (stemRatio >= .82) why.push("close filenames");
...
function representatives(copies, groups) {
...
function similarClusters(copies, groups, skipKeys = []) {
	...
	const whyByRoot = /* @__PURE__ */ new Map();
	const scoreByRoot = /* @__PURE__ */ new Map();
...
function playlistDuplicateHints(copies, groups = [], skipKeys = []) {
	const skip = new Set(skipKeys);
	const marks = /* @__PURE__ */ new Map();
	const mark = (ids, why) => {
		for (const id of ids) {
			if (ids.filter((other) => other !== id && !skip.has(skipPairKey(id, other))).length === 0) continue;
	...
	for (const cluster of similarClusters(copies, groups, skip)) mark(cluster.copies.map((copy) => copy.track.id), cluster.why?.join(" · ") || "similar names");
...
//#endregion
export { similarClusters as n, similar_cuts_BE0d4_OA_exports as r, playlistDuplicateHints as t };
