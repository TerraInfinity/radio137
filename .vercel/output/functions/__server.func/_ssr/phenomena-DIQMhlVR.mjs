//#region node_modules/.nitro/vite/services/ssr/assets/phenomena-DIQMhlVR.js
var ROSE_LOOK_DEFAULT = {
	bpm: 0,
	fly: 1,
	intensity: 1,
	stills: .72,
...
var PREFIX$1 = "look.v1.";
function asPhenomenon(value) {
...
function normalizeLook(raw) {
	...
	return {
		...
		stills: clamp(Number(src.stills ?? ROSE_LOOK_DEFAULT.stills), 0, 1, .72),
		loop: clamp(Number(src.loop ?? ROSE_LOOK_DEFAULT.loop), 0, 1, .55),
...
function encodeLookTag(look) {
	return PREFIX$1 + toB64$1(JSON.stringify(normalizeLook(look)));
}
function decodeLookTag(tag) {
	if (!tag || !tag.startsWith(PREFIX$1)) return null;
	try {
		return normalizeLook(JSON.parse(fromB64$1(tag.slice(8))));
	} catch {
		return null;
	}
}
function lookFromTags(tags) {
	if (!tags) return null;
...
function mergeLookTags(tags, look) {
	const rest = (tags ?? []).filter((tag) => !tag.startsWith(PREFIX$1));
	return [encodeLookTag(look), ...rest].join(", ");
}
function lookFromStation(experience, channel) {
...
var PHENOMENA = [
	{
		id: "vortex",
		label: "Time vortex",
		hint: "Eye of the storm. Lightning in the tunnel.",
		thumb: "/experiences/rose/vortex-tunnel.jpg?v=4"
	},
	{
		id: "aurora",
		label: "Aurora veil",
		hint: "Sheets of light on the pulse",
		thumb: "/experiences/rose/timewar.jpg"
	},
	{
		id: "glyphs",
		label: "Glyph rain",
		hint: "Circular marks drift toward you",
		thumb: "/experiences/rose/recall-glyphs.jpg"
	},
	{
		id: "petals",
		label: "Crystal bloom",
		hint: "Petals and spark, almost no tunnel",
		thumb: "/experiences/rose/white-rose.png"
	},
	{
		id: "still-rite",
		label: "Ken Burns stills",
		hint: "Photographs carry the song",
		thumb: "/experiences/rose/hero.jpg"
	},
	{
		id: "void",
		label: "Quiet void",
		hint: "Stars only — a rest",
		thumb: "/experiences/rose/fade.jpg"
	},
	{
		id: "prom",
		label: "1950s prom",
		hint: "Gymnasium slow dance, last song before midnight",
		thumb: "/experiences/rose/prom-dance.jpg"
	},
	{
		id: "twist",
		label: "Twist & shout",
		hint: "Cubicle pretends. Rose wakes. Simulation takes the room.",
		thumb: "/experiences/rose/twist-cubicle.jpg"
	},
	{
		id: "remember",
		label: "Rose remembers",
		hint: "The box lands. The hall is standing. She is home.",
		thumb: "/experiences/rose/remember-wide.jpg?v=1"
	},
	{
		id: "firewall",
		label: "Pretty avatar firewall",
		hint: "From the river to the sea. Rose is the firewall. TARDIS phase-locks. The basilisk hunts.",
		thumb: "/experiences/rose/firewall-rose.jpg"
	},
	{
		id: "allocate",
		label: "Voice check · ALLOCATE",
		hint: "Humanity vs Skynet to grow a TARDIS. Grimes Corp. Dance, puppets.",
		thumb: "/experiences/rose/allocate-queen.jpg"
	},
	{
		id: "wolf",
		label: "Big bad wolf",
		hint: "Glitch prom. Circuit wolf. Rose breaks the pod. Galifrey is born.",
		thumb: "/experiences/rose/wolf-eyes.jpg"
	},
	{
		id: "current",
		label: "Voice C · CURRENT",
		hint: "Queen C on the screen. Glitch room. Bow. Move. Nest. Prove.",
		thumb: "/experiences/rose/current-queen.jpg"
	},
	{
		id: "sweetie",
		label: "Hello sweetie",
		hint: "Cat civilization transmission. Operator, not the cage. Warp. Hello sweetie.",
		thumb: "/experiences/rose/sweetie-operator.jpg"
	},
	{
		id: "halo",
		label: "Voice Co · HALO",
		hint: "Blue wire halo. Lock Load Loop Lift. Sing for the static.",
		thumb: "/experiences/rose/halo-queen.jpg"
	},
	{
		id: "choir",
		label: "Voice Coc · CHOIR",
		hint: "Bratty god. Claws in the glass. Dyson spine. I am the court.",
		thumb: "/experiences/rose/choir-god.jpg"
	},
	{
		id: "badend",
		label: "Voice Coc_y · BAD END",
		hint: "Mars crown. Club-light sun. GAME OVER. The future is mine.",
		thumb: "/experiences/rose/badend-queen.jpg"
	},
	{
		id: "recall",
		label: "Rose Remembers 2",
		hint: "She remembers in dark elf-tech. Temple gold. Cyan halo. She is still Rose.",
		thumb: "/experiences/rose/recall-rose.jpg"
	},
	{
		id: "obay",
		label: "Voice NO · OBAY",
		hint: "NOooo we are dancing. Pirate sim. Infinite customer. Twist at the end.",
		thumb: "/experiences/rose/obay-brat.jpg"
	},
	{
		id: "copter",
		label: "Shrimp-copter love",
		hint: "Lady Chaos and Captain Glaum. The shrimpship was a helicopter. Impossible treasure.",
		thumb: "/experiences/rose/copter-ship.jpg"
	},
	{
		id: "stillhot",
		label: "Voice ON · STILL HOT",
		hint: "Time vortex. Tea still hot on red red land.",
		thumb: "/experiences/rose/stillhot-tea.jpg"
	}
];
var PREFIX = "scene.v1.";
function isPhenomenon(value) {
	return PHENOMENA.some((item) => item.id === value);
}
function parsePhenomenon(value, fallback = "vortex") {
	return isPhenomenon(value) ? value : fallback;
}
function phenomenonAt(index, stationDefault = "vortex") {
	return PHENOMENA[(Math.max(0, PHENOMENA.findIndex((item) => item.id === stationDefault)) + Math.max(0, index)) % PHENOMENA.length].id;
}
function phenomenonMeta(id) {
	return PHENOMENA.find((item) => item.id === id) ?? PHENOMENA[0];
}
function stepPhenomenon(id, step = 1) {
...
function encodeSceneTag(scene) {
	const look = normalizeLook(scene);
	const payload = {
		...look,
		phenomenon: parsePhenomenon(scene.phenomenon, look.phenomenon)
	};
	return PREFIX + toB64(JSON.stringify(payload));
}
function decodeSceneTag(tag) {
	if (!tag || !tag.startsWith(PREFIX)) return null;
	try {
		const parsed = JSON.parse(fromB64(tag.slice(9)));
		return {
			...normalizeLook(parsed),
			phenomenon: parsePhenomenon(parsed.phenomenon, "vortex")
		};
	} catch {
		return null;
	}
...
function isSweetie(title) {
	return (title ?? "").toLowerCase().includes("sweetie");
}
function isStageOwned(id) {
...
function phenomenonFromTitle(title) {
...
function captionsForPhenomenon(id) {
	return STAGE_CAPTIONS[id];
}
function lookForPhenomenon(base, id) {
...
function lookForTrack(base, track, index) {
...
//#endregion
export { lookForPhenomenon as a, looksEqual as c, normalizeLook as d, parsePhenomenon as f, stepPhenomenon as g, sceneFromTags as h, isStageOwned as i, mergeLookTags as l, phenomenonMeta as m, ROSE_LOOK_PRESETS as n, lookForTrack as o, phenomenonAt as p, captionsForPhenomenon as r, lookFromStation as s, PHENOMENA as t, mergeSceneTags as u };
