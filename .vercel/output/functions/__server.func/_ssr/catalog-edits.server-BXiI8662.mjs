...
import { i as r2KeyFromAudioUrl } from "./file-path-PXlvogCB.mjs";
import { r as getSql } from "./db-BsyRipWH.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/catalog-edits.server-BXiI8662.js
...
async function liveCatalog() {
	const { getSeedCatalog } = await import("./catalog-BcaLbP39.mjs").then((n) => n.t);
	const { applyCatalogEdits } = await import("./catalog-edits-BhjK6UvC.mjs").then((n) => n.n).then((n) => n.n);
...
async function placeTrack(user, input) {
	...
	if (to.tracks.some((item) => item.enabled !== false && item.audioUrl && item.audioUrl === track.audioUrl)) throw new Error(`Already on ${to.name}`);
	await addTrack(user, {
...
//#endregion
export { patchTrack as a, listStationEdits as i, catalog_edits_server_exports as n, listEdits as r, addTrack as t };
